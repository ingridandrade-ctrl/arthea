...
export function RecorrenciasClient() {
  ...
  async function loadAll() {
    ...
    const [r, a, c, s] = await Promise.all([
      fetch("/api/financas/recurring").then((r) => r.json()),
  ...
  async function toggleActive(rule: Rule) {
    await fetch(`/api/financas/recurring/${rule.id}`, {
      method: "PATCH",
  ...
  async function remove(id: string) {
    ...
    await fetch(`/api/financas/recurring/${id}`, { method: "DELETE" });
    loadAll();
  }

  async function runNow() {
    setRunning(true);
    const res = await fetch("/api/financas/recurring/run", { method: "POST" });
...
function RuleModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = editing ? `/api/financas/recurring/${rule!.id}` : "/api/financas/recurring";
    const payload: any = {
      name,
